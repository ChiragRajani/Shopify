...
app.get("/", (req, res) => {
    if (req.query.search) {
        ...
        const check = req.query.search.replace("+", " ").split("?");;
        console.log(check)
        product.find({ "name": { $in: check } }, function(err, product) {
            if (err) {
                console.log(err);
            } else {
                res.render("show2", { user: req.user, product: product });
            }
...
app.get("/filters/:id", (req, res) => {
    ...
    product.find({ category: cate }, function(err, product) {
        if (err) {
            console.log(err);
        } else {
            res.render("show", { user: req.user, product: product });
